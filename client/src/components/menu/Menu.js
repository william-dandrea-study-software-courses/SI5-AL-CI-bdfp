          ...
          <Grid container rowSpacing={1} columnSpacing={{ xs: 1, sm: 2, md: 3 }}>
            {allItems.map((x) => (
              <Grid item xs={6} key={x._id}>
                <MenuItem menuItem={x} addToCart={addToCart} removeFromCart={removeFromCart} numberOfItemsP={numberOfItemsP} />
              </Grid>
            ))}
          </Grid>

            <Button size="small" color={"success"} onClick={validateOrder} >Place Order</Button>
        </div>
    );
});
